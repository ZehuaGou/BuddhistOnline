      ...
      <table class="table table-striped order-table">
        ...
        <c:forEach items="${orderFormList}" var="orderForm" varStatus="status">
            <tr>
              ...
              <td>
                <a href="${pageContext.request.contextPath}/orderForm/jumpOrderFormDetails/${orderForm.oid}">
                    ${orderForm.inventory.designation.name}
                </a>
              </td>
              <td>${orderForm.inventory.designation.price}</td>

              <%--标记为Y则显示被指定，标记为N则显示没被指定--%>
              <c:if test="${orderForm.flag == 'Y'}">
                <td class="td3 td-green">是</td>
              </c:if>
              <c:if test="${orderForm.flag == 'N'}">
                <td class="td3 td-red">否</td>
              </c:if>

              <td>
                <a href="${pageContext.request.contextPath}/orderForm/deleteOrderForm/${orderForm.oid}">
                  删除
                </a>
              </td>
            </tr>
    ...
    <div class="ann">
      ...
      <c:if test="${pageInfo.pageNum > 1}">
        <button class="btn btn-info ann-btn"
                onclick="window.location.href='${pageContext.request.contextPath}/orderForm/jumpOrderFormList/1'">
          首页
        </button>
        <button class="btn btn-info ann-btn"
                onclick="window.location.href='${pageContext.request.contextPath}/orderForm/jumpOrderFormList/${pageInfo.pageNum - 1}'">
          上一页
        </button>
      </c:if>
      <c:if test="${pageInfo.pageNum < pageInfo.lastPage}">
        ...
        <button class="btn btn-info ann-btn"
                onclick="window.location.href='${pageContext.request.contextPath}/orderForm/jumpOrderFormList/${pageInfo.lastPage}'">
          尾页
        </button>
      </c:if>
    </div>
